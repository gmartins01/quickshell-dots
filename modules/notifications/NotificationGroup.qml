...
MouseArea { // Notification group area
    ...
    onContainsMouseChanged: {
        ...
        if (root.containsMouse)
            root.notifications.forEach(notif => {
                NotificationService.cancelTimeout(notif.notificationId);
            });
        else
            root.notifications.forEach(notif => {
                NotificationService.timeoutNotification(notif.notificationId);
            });
    }
    ...
    SequentialAnimation { // Drag finish animation
        ...
        onFinished: () => {
            root.notifications.forEach(notif => {
                Qt.callLater(() => {
                    NotificationService.discardNotification(notif.notificationId);
                });
            });
        }
    }
    ...
    Rectangle { // Background of the notification
        ...
        width: parent.width
        color: popup ? ColorUtils.applyAlpha(Colors.colors.colLayer2, 1 - Appearance.backgroundTransparency) : Colors.colors.colLayer2
                ...
                Item { // App name (or summary when there's only 1 notif) and time
                    ...
                    RowLayout {
                        ...
                        StyledText {
                            ...
                            color: topRow.showAppName ? Colors.colors.colSubtext : Colors.colors.colOnLayer2
                        }
                        StyledText {
                            id: timeText
                            // Layout.fillWidth: true
