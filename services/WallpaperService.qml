...
Singleton {
    ...
    Component.onCompleted: {
        ...
        var monitors = Settings.options.wallpaper.monitors || [];
    ...
    function getFillModeUniform() {
        for (var i = 0; i < fillModeModel.count; i++) {
            ...
            if (mode.key === Settings.options.wallpaper.fillMode) {
                return mode.uniform;
            }
        }
        // Fallback to crop
    ...
    //         if (!Settings.options.wallpaper.enableMultiMonitorDirectories) {
    //             // All monitors use the main directory
    //             for (var i = 0; i < Quickshell.screens.length; i++) {
    //                 root.wallpaperDirectoryChanged(Quickshell.screens[i].name, Settings.options.wallpaper.directory);
    //             }
    //         } else {
    ...
    //                     root.wallpaperDirectoryChanged(screenName, Settings.options.wallpaper.directory);
    //                 }
    //             }
    //         }
    //     }
    ...
    //     if (!Settings.options.wallpaper.enableMultiMonitorDirectories) {
    //         return Settings.options.wallpaper.directory;
    //     }
    //
    ...
    //     Settings.options.wallpaper.monitors = newMonitors.slice();
    ...
    function _setWallpaper(screenName, path) {
        ...
        if (screenName === undefined) {
            console.log("Wallpaper ", "setWallpaper", " no screen specified");
            return;
        }
    ...
    //     console.log.log("Wallpaper ", "setRandomWallpaper");
    //
    //     if (Settings.options.wallpaper.enableMultiMonitorDirectories) {
    ...
    //     console.log("Wallpaper ", "toggleRandomWallpaper");
    //     if (Settings.options.wallpaper.randomEnabled) {
    ...
    //     if (Settings.options.wallpaper.isRandom) {
    //         randomWallpaperTimer.restart();
    //     }
    // }
    ...
    //     console.log("Wallpaper ", "refreshWallpapersList");
    //     scanningCount = 0;
    //
    ...
    //     interval: Settings.options.wallpaper.randomIntervalSec * 1000
    //     running: Settings.options.wallpaper.randomEnabled
    //     repeat: true
    ...
    //
    //                 scanningCount--;
    //                 console.log("Wallpaper ", "List refreshed for ", screenName, " count: ", files.length);
    ...
    IpcHandler {
        ...
        function set(path: string): void {
            root.changeWallpaper(path, undefined);
        }
    }
}
